...
class SystemController extends Controller
{
    ...
    public function choose_shipping_address_other(Request $request) {
        ...
        // start coding for pinkwally     
        if (!Auth::guard('customer')->check()) {
     

                $shippingString = $request->input('shipping');
                parse_str($shippingString, $shippingDetails);

                // URL decode and sanitize inputs
                $name = urldecode($shippingDetails['contact_person_name'] ?? '');
                $phone = $shippingDetails['phone'] ?? '';
                $email = urldecode($shippingDetails['email'] ?? '');
                $address = urldecode($shippingDetails['address'] ?? '');
                ...
                $p_is_exist_user = User::where('phone', $phone)->orWhere('email', $email)->first();

                if ($p_is_exist_user) {
                    Auth::guard('customer')->login($p_is_exist_user);
                    CartManager::cart_to_db();
                    return response()->json(['exist user login' => 'Success'], 200);
                }


                // Create user if email and phone are both available and user does not exist
                $user = User::create([
                    'name' => $name,
                    'phone' => $phone,
                    'email' => $email,
